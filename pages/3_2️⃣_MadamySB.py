...
def conexãoSB():
    ...
    conexaoFilial = mysql.connector.connect(
        host="srv496.hstgr.io",
        database="u771541202_db_filial",
        user="u771541202_madamy_filial",
    ...
    if conexaoFilial.is_connected():
        ...
        dfCaixa = pd.read_sql(filialquerycaixa, conexaoFilial)
        dfCaixaItens = pd.read_sql(filialquerycaixaItens, conexaoFilial)
        dfCargos = pd.read_sql(filialquerycargos, conexaoFilial)
        dfClientes = pd.read_sql(filialqueryclientes, conexaoFilial)
        dfColaboradores = pd.read_sql(filialquerycolaboradores, conexaoFilial)
        dfComandas = pd.read_sql(filialquerycomandas, conexaoFilial)
        dfEmpresas = pd.read_sql(filialqueryempresas, conexaoFilial)
        dfEstoques = pd.read_sql(filialqueryestoques, conexaoFilial)
        dfFormaPagamento = pd.read_sql(filialqueryforma_pagamentos, conexaoFilial)
        dfFornecedores = pd.read_sql(filialqueryfornecedores, conexaoFilial)
        dfGrupoProdutos = pd.read_sql(filialquerygrupo_produtos, conexaoFilial)
        dfItemComandas = pd.read_sql(filialqueryitem_comandas, conexaoFilial)
        dfItemEstoque = pd.read_sql(filialqueryitem_estoques, conexaoFilial)
        dfLojas = pd.read_sql(filialquerylojas, conexaoFilial)
        dfProdutos = pd.read_sql(filialqueryprodutos, conexaoFilial)
        dfProdutoPreco = pd.read_sql(filialqueryproduto_precos, conexaoFilial)
        dfPromocoes = pd.read_sql(filialquerypromocaos, conexaoFilial)
